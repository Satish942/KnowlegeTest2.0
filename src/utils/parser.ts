  ...
  for (let i = 0; i < markers.length; i++) {
    ...
    if (cleanAns) {
      ...
      if (ansHit) {
        ...
        // 1. Check for consecutive letter clusters (e.g. "AC", "ABC")
        // We look for 2 to 6 uppercase letters A-H that form a standalone word.
        const clusterMatch = line.match(/\b([A-H]{2,6})\b/i);
        
        if (clusterMatch && /^[A-H]+$/i.test(clusterMatch[1])) {
          arr = clusterMatch[1].toUpperCase().split('');
        } else {
          ...
          arr = letters.length > 0 ? letters : digits;
        }
